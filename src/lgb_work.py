...
if __name__ == '__main__':
    ...
    test = pd.read_csv(r'E:\PycharmProjects\supply-chain\TestSets\TestPlus.csv')
    ...
    gbm.save_model(r'E:\PycharmProjects\supply-chain\models\model1.txt')
    ...
    lgb_eval = lgb.Dataset(X_test, y_test, reference=lgb_train)
    gbm = lgb.train(params, lgb_train, num_boost_round=5000, valid_sets=lgb_eval, early_stopping_rounds=100)
    gbm.save_model(r'E:\PycharmProjects\supply-chain\models\model2.txt')
    y_pred = gbm.predict(test[test.columns[2:]], num_iteration=gbm.best_iteration)
    # y_pred = y_pred.astype('int')
    ...
    lgb_eval = lgb.Dataset(X_test, y_test, reference=lgb_train)
    gbm = lgb.train(params, lgb_train, num_boost_round=5000, valid_sets=lgb_eval, early_stopping_rounds=100)
    gbm.save_model(r'E:\PycharmProjects\supply-chain\models\model3.txt')
    y_pred = gbm.predict(test[test.columns[2:]], num_iteration=gbm.best_iteration)
    # y_pred = y_pred.astype('int')
    ...
    lgb_eval = lgb.Dataset(X_test, y_test, reference=lgb_train)
    gbm = lgb.train(params, lgb_train, num_boost_round=5000, valid_sets=lgb_eval, early_stopping_rounds=100)
    gbm.save_model(r'E:\PycharmProjects\supply-chain\models\model4.txt')
    y_pred = gbm.predict(test[test.columns[2:]], num_iteration=gbm.best_iteration)
    # y_pred = y_pred.astype('int')
    ...
    lgb_eval = lgb.Dataset(X_test, y_test, reference=lgb_train)
    gbm = lgb.train(params, lgb_train, num_boost_round=5000, valid_sets=lgb_eval, early_stopping_rounds=100)
    gbm.save_model(r'E:\PycharmProjects\supply-chain\models\model5.txt')
    y_pred = gbm.predict(test[test.columns[2:]], num_iteration=gbm.best_iteration)
    # y_pred = y_pred.astype('int')
    test['week5'] = pd.Series(y_pred)

    test = test[['sku_id', 'week1', 'week2', 'week3', 'week4', 'week5']]
    test.to_csv(r'E:\PycharmProjects\supply-chain\output\lgb_reault.csv', index=False)
